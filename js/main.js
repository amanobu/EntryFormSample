$(function(){
	var setWrap = $('#container'),
	    setBase = $('.stageBase'),//縦スライド
	    scrollSpeed = 10,
	    scrollEasing = 'swing',
	    slideSpeed = 10,
	    slideEasing = 'linear',
	    downBtn = 'show', // 'show' or 'hide'
	    urlHash = 'on', //  'on' or 'off' onにするとページ内リンクが作られ、ページ直アクセスが可能となる?→そんなこともなさそう
	    setHash = '!page';//↑のhttp://～.html#!page?とする為のキーワードを何にするかということ
    
	var url = document.URL,
	    stageSlide = $('.stageSlide');
    //縦方向のナビゲーション①
	setWrap.append('<nav id="pageNav"><ul></ul></nav>');
    //ページの枚数に応じて縦方向のページへのリンクを追加していく②
	...
	var coreNav = $('#pageNav'),
	    setNav = coreNav.find('ul'), //①のラグ
	    navList = setNav.find('li'), //②で追加たタグ
	    navLength = navList.length; //ページは何個か
	setNav.find('li:first').addClass('activeStage'); //一番最初のページにactiveStageのclassを追加
	$('body').attr('data-page','1'); //これは何でつけるんだろうか
    
	$(window).load(function(){//読み込まれたら処理がされる
		// StageHeight
		$(window).resize(function(){//リサイズのイベント
		    var wdHeight = $(window).height();
		    setBase.css({height:wdHeight});//windowsサイズを縦横最大にする？
        ...
        //#pageDownを表示するかしないかを判定し、display:をいじる
		function pagePos(){
		    var pnAcv = coreNav.find('li.activeStage');//coreNav:$('#pageNav') アクティブのページのを探す
		    pnAcv.each(function(){
		...
		function replaceHash(){
		    var pnAcv = coreNav.find('li.activeStage');//coreNav:$('#pageNav') アクティブのページのを探す
		    pnAcv.each(function(){//#pageNavの要素ごとに
			    var pnIndexN = navList.index(this),
				    pnCountN = pnIndexN+1;
